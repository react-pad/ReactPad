...
export default function VotingPage() {
  const [selectedVotes, setSelectedVotes] = useState<{ [key: number]: 'for' | 'against' | null }>({});
  const [filter, setFilter] = useState<'all' | 'active' | 'passed' | 'rejected'>('all');

  const handleVote = (proposalId: number, vote: 'for' | 'against') => {
    setSelectedVotes(prev => ({ ...prev, [proposalId]: vote }));
  };
        ...
        <div className="space-y-6">
          {filteredProposals.map((proposal) => {
            ...
            return (
              <div
                key={proposal.id}
                className="border-4 border-black p-8 bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all duration-200"
              >
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        className={`px-6 py-3 text-sm font-black uppercase border-4 border-black transition-all ${
                          selectedVotes[proposal.id] === 'for'
                            ? 'bg-[#2FFF2F] text-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] translate-x-[-2px] translate-y-[-2px]'
                      ...
                      <button
                        ...
                        className={`px-6 py-3 text-sm font-black uppercase border-4 border-black transition-all ${
                          selectedVotes[proposal.id] === 'against'
                            ? 'bg-[#FF00F5] text-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] translate-x-[-2px] translate-y-[-2px]'
                            : 'bg-[#FFFF00] text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px]'
                        }`}
                      >
                        VOTE AGAINST
                      </button>
